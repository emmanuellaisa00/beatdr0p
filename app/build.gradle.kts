...
android {
    ...
    // Falls back to null so debug builds always work without any secrets.
    signingConfigs {
        create("release") {
            val ksPath = System.getenv("KEYSTORE_FILE")
                ?: (rootProject.file("keystore.properties").takeIf { it.exists() }?.let {
                    val p = java.util.Properties(); it.inputStream().use { s -> p.load(s) }; p.getProperty("storeFile")
                })
            if (ksPath != null && file(ksPath).exists()) {
                val props = rootProject.file("keystore.properties").takeIf { it.exists() }?.let {
